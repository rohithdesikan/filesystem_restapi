...
# Create the test_folder location as an environment variable
...
def test_existing_root_folder():
    response = client.get("/")
    assert response.status_code == 200
    assert isinstance(response.json(), dict)

# Test if hidden files are shown
def test_existing_root_folder_contents():
    response = client.get("/")
    assert response.status_code == 200
    rjson = response.json()
...
def test_sub_file():
    response = client.get("/filecontents.txt")
    assert response.status_code == 200
    rjson = response.json()
...
def test_sub_file_no_exists():
    response = client.get("/test_noexist.txt")
...
def test_open_file_nottxt():
    response = client.get("/nontxtfile.py")
...
def test_sub_folder():
    response = client.get("/foldercontents")
    assert response.status_code == 200
    rjson = response.json()
    assert rjson['is_folder'] == True
...
def test_sub_folder_blank():
    response = client.get("/blankfolder")
    assert response.status_code == 200
    rjson = response.json()
    assert rjson['is_folder'] == True
...
def test_create_folder_exists():
    response = client.post("/createfolder", json = {'create_name': 'existingfolder'})
    assert response.status_code == 422
    rjson = response.json()
    assert rjson['detail'] == 'Folder already exists, no action taken'

def test_create_file():
    response = client.post("/createfile", json = {'create_name': 'newfile.txt', 'create_content': 'This is a sample text file that is generated with a create POST request'})
    assert response.status_code == 200
    assert 'newfile.txt' in os.listdir(test_path)

def test_create_file_exists():
    response = client.post("/createfile", json = {'create_name': 'existingfolder/existingfile.txt', 'create_content': 'This is a sample text file that is generated with a create POST request'})
    assert response.status_code == 422
    rjson = response.json()
    assert rjson['detail'] == 'File already exists, no action taken'

def test_create_file_nontxt():
    response = client.post("/createfile", json = {'create_name': 'newfile.py', 'create_content': 'This is a sample non text file for expected error'})
    assert response.status_code == 422
    rjson = response.json()
...
# --------------------------------------------------------
def test_empty_file():
    response = client.delete("/emptyfolder", json = {'delete_name': 'emptyfoldercontents/test1.txt'})
    assert response.status_code == 422
    rjson = response.json()
    assert rjson['detail'] == 'Note that empty_folder is only to empty a folder, not to empty the contents of files'

def test_empty_folder_noexist():
    response = client.delete("/emptyfolder", json = {'delete_name': 'test_no_exist'})
    assert response.status_code == 404

def test_empty_folder():
    response = client.delete("/emptyfolder", json = {'delete_name': 'emptyfoldercontents'})
    assert response.status_code == 200
    rjson = response.json()
...
def test_delete_folder():
    response = client.delete("/deletefolder", json = {'delete_name': 'delete_folder'})
    assert response.status_code == 200
    rjson = response.json()
    assert rjson['detail'] == 'Folder Deleted Successfully'

def test_delete_folder_noexist():
    response = client.delete("/deletefolder", json = {'delete_name': 'folder_no_exist'})
    assert response.status_code == 404

# Try deleting a non empty folder to make sure the right result is returned
def test_delete_folder_notempty():
    response = client.delete("/deletefolder", json = {'delete_name': 'foldercontents'})
    assert response.status_code == 422
    rjson = response.json()
...
def test_delete_file():
    response = client.delete("/deletefile", json = {'delete_name': 'test_delete_file.txt'})
    assert response.status_code == 200
    rjson = response.json()
    assert rjson['detail'] == 'File Deleted Successfully'

def test_delete_file_noexist():
    response = client.delete("/deletefile", json = {'delete_name': 'file_no_exist.txt'})
    assert response.status_code == 404
